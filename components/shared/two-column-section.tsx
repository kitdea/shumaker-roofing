...
const richTextOptions = {
  renderNode: {
    [BLOCKS.HEADING_2]: (_node: unknown, children: React.ReactNode) => (
      <h2 className="text-xl font-heading font-bold text-foreground mt-6 first:mt-0 mb-2">{children}</h2>
    ),
    [BLOCKS.HEADING_3]: (_node: unknown, children: React.ReactNode) => (
      <h3 className="text-lg font-heading font-semibold text-foreground mt-5 first:mt-0 mb-1.5">{children}</h3>
    ),
    [BLOCKS.HEADING_4]: (_node: unknown, children: React.ReactNode) => (
      <h4 className="text-base font-heading font-semibold text-foreground mt-4 mb-1">{children}</h4>
    ),
    ...
    [INLINES.HYPERLINK]: (node: unknown, children: React.ReactNode) => {
      ...
      return (
        <a href={uri} className="text-primary underline underline-offset-4 hover:opacity-80 transition-opacity" target="_blank" rel="noopener noreferrer">
          {children}
        </a>
      );
    },
    ...
    <div className="flex flex-col justify-center gap-5">
      ...
      {splitDescription && (
        <div className="space-y-1 text-base md:text-[1.0rem]">
          {documentToReactComponents(splitDescription as Document, richTextOptions as Parameters<typeof documentToReactComponents>[1])}
        </div>
      )}
    </div>
  );
